                ...
                <div class="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
                    <div>
                        ...
                        <p class="mt-3 max-w-xl text-sm opacity-70">
                            Only your own win or lose result is shown here. Other players’ names and tickets stay completely private.
                        </p>
                    </div>
            ...
            <form method="GET" action="{{ route('lotteries.results', $lottery) }}" class="p-5 md:p-8">
                ...
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-6">
                    <div class="md:col-span-2 xl:col-span-6">
                        <label for="lottery-results-search" class="mb-2 block text-xs uppercase tracking-wider opacity-50">
                            Search my tickets
                        </label>
                        <div class="relative">
                    ...
                    <div>
                        <label for="lottery-results-round" class="mb-2 block text-xs uppercase tracking-wider opacity-50">Round</label>
                        <input
                            id="lottery-results-round"
                            type="number"
                            name="round"
                            min="1"
                            max="{{ $totalRounds }}"
                    ...
                    <div>
                        <label for="lottery-results-prize" class="mb-2 block text-xs uppercase tracking-wider opacity-50">My prize</label>
                        <select
                            id="lottery-results-prize"
                            name="prize"
                    ...
                    <div>
                        <label for="lottery-results-sort" class="mb-2 block text-xs uppercase tracking-wider opacity-50">Sort</label>
                        <select
                            id="lottery-results-sort"
                            name="sort"
                            class="w-full rounded-2xl border border-brand-border bg-brand-dark px-4 py-3.5 text-sm text-white outline-none transition focus:border-green-500/40"
                        >
                    ...
                    <div>
                        <label for="lottery-results-from" class="mb-2 block text-xs uppercase tracking-wider opacity-50">From</label>
                        <input
                            id="lottery-results-from"
                            type="date"
                            name="from"
                            value="{{ $filters['from'] }}"
                    ...
                    <div>
                        <label for="lottery-results-to" class="mb-2 block text-xs uppercase tracking-wider opacity-50">To</label>
                        <input
                            id="lottery-results-to"
                            type="date"
                            name="to"
                            value="{{ $filters['to'] }}"
                            class="w-full rounded-2xl border border-brand-border bg-brand-dark px-4 py-3.5 text-sm text-white outline-none transition [color-scheme:dark] focus:border-green-500/40"
                    ...
                    <div class="flex items-end">
                        <label class="flex h-[50px] w-full cursor-pointer items-center gap-3 rounded-2xl border border-brand-border bg-brand-dark px-4 text-sm text-white">
                            <input
                                type="checkbox"
                                name="my_wins"
                                value="1"
                                @checked($filters['my_wins'])
            ...
            @if ($draws->count())
                <div class="space-y-3">
                    @foreach ($draws as $draw)
                        ...
                        <details
                            id="draw-{{ $draw->id }}"
                            class="group overflow-hidden rounded-3xl border border-brand-border bg-brand-surface open:border-green-500/30"
                            @if ($isFocused) open @endif
                        >
                            <summary
                                class="flex cursor-pointer list-none items-center gap-4 p-4 transition hover:bg-white/[0.02] md:p-5 [&::-webkit-details-marker]:hidden"
                            >
                                <div class="flex h-12 w-12 shrink-0 flex-col items-center justify-center rounded-2xl border border-orange-500/20 bg-orange-500/10">
                                    <span class="text-[10px] uppercase tracking-wider text-orange-400/80">Rnd</span>
                                    <span class="text-lg font-bold leading-none text-orange-400">{{ $roundNumber ?? '—' }}</span>
                                </div>

                                <div class="min-w-0 flex-1">
                                    <div class="flex flex-wrap items-center gap-2">
                                        <h3 class="font-bold">Round {{ $roundNumber ?? $draw->id }}</h3>
                                        @if ($isLatest)
                                            <span class="rounded-full border border-green-500/20 bg-green-500/10 px-2.5 py-0.5 text-[11px] font-semibold text-green-400">
                                                Latest
                                            </span>
                                        @endif
                                        ...
                                        @elseif ($personal['outcome'] === 'lost')
                                            <span class="rounded-full border border-orange-500/20 bg-orange-500/10 px-2.5 py-0.5 text-[11px] font-semibold text-orange-400">
                                                No win
                                            </span>
                                        @else
                                            <span class="rounded-full border border-brand-border bg-brand-dark px-2.5 py-0.5 text-[11px] font-semibold opacity-50">
                                                Not entered
                                            </span>
                                        @endif
                                    </div>
                                    <p class="mt-1 truncate text-sm opacity-60">
                                        {{ $draw->completed_at?->format('d M Y · h:i A') ?? '—' }}
                                        · {{ number_format($draw->total_tickets) }} tickets in round
                                    </p>
                                </div>

                                <i class="fa-solid fa-chevron-down shrink-0 text-sm opacity-40 transition group-open:rotate-180"></i>
                            </summary>

                            <div class="border-t border-brand-border px-4 pb-4 pt-4 md:px-5 md:pb-5">
                                ...
                                <div class="mt-4 flex justify-end">
                                    <a
                                        href="{{ route('lotteries.draws.show', [$lottery, $draw]) }}"
                                        class="inline-flex items-center gap-2 text-xs font-semibold text-green-400 transition hover:underline"
                                    >
                ...
                @endif
            @else
                <div class="rounded-3xl border border-dashed border-brand-border bg-brand-surface/50 px-6 py-16 text-center">
                    <div class="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl border border-orange-500/20 bg-orange-500/10">
